...
class Deck {
    ...
    // Randomly shuffles the deck, swapping card indexes - used shuffle funciton
    shuffle() {
    ...
    //Split deck to both players 26 cards per player
...
class Game {
    ...
    // A prompt will ask you to assign the names of players. Will not accept null or empty string.
    createPlayer(player) {
        let name = prompt(`Enter name of player ${player}.`, `Player ${player}`);
                
        while (name == '' || name === null) {
            name = prompt(`Who is playng? ${player}.`);
        }
